...
def name_trial(trial):
    ...
    return f'agentpool-{args.num_learners}x{trial.trainable_name}'
...
tune.run(
    ...
    stop={
        ...
        # 'timesteps_total': int(1e9) * args.num_learners,
    },
    config=dict({
        'env': 'c4',
        'multiagent': {
            'policies_to_train': [*trainable_policies],
            'policy_mapping_fn': lambda _: random.sample([*trainable_policies], k=2),
            'policies': {
                **trainable_policies,
                **mcts_train_policies,
